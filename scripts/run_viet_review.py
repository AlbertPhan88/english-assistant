"""One-shot script: review all stored Vietnamese mappings in parallel.

Run with: .venv/bin/python -m scripts.run_viet_review
"""
...
# Haiku tier-1 limit is 50 RPM. Stay safely under it.
MAX_RPM = 40
...
def _gate() -> None:
    """Block until the next request is allowed under MAX_RPM."""
...
def review_with_retry(phrase: str, meaning: str, viet: str, client: Anthropic) -> str:
    """Same logic as src.examples.review_vietnamese_equiv but rate-limited + retried."""
    backoff = 5.0
    for attempt in range(6):
        ...
        try:
            ...
            print(f"  ! 429 on {phrase!r}, backing off {backoff:.0f}s (attempt {attempt+1})", flush=True)
            time.sleep(backoff)
...
def main() -> int:
    ...
    with db.connect(config.DB_PATH) as conn:
        rows = list(conn.execute(
            ...
            "AND vietnamese_equiv != '—'"
        ))
    total = len(rows)
    print(f"Reviewing {total} idioms — rate limit {MAX_RPM} RPM, concurrency=4 ...", flush=True)
    t0 = time.monotonic()
    updated = 0
    done = 0

    def task(row: sqlite3.Row) -> tuple[int, str, str, str]:
        improved = review_with_retry(
